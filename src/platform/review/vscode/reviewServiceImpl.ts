...
const reviewDiffContextKey = 'jiido.chat.reviewDiff.enabled';
const reviewDiffReposContextKey = 'jiido.chat.reviewDiff.enabledRootUris';
const numberOfReviewCommentsKey = 'jiido.chat.review.numberOfComments';
...
export class ReviewServiceImpl implements IReviewService {
	...
	constructor(
		...
		this._disposables.add(vscode.workspace.onDidChangeConfiguration(e => {
			...
			if (e.affectsConfiguration('jiido.advanced') || e.affectsConfiguration('jiido.advanced.review.intent')) {
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = vscode.languages.createDiagnosticCollection('jiido.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (this._comments.length === 1) {
				vscode.commands.executeCommand('jiido.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					...
					if (active !== this._activeThread) {
						...
						if (active) {
							vscode.commands.executeCommand('jiido.chat.review.current', active);
						}
					}
				}, 500);
			}
