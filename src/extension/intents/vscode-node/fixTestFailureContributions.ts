...
function registerTestMessageSparkles(store: DisposableStore, telemetryService: ITelemetryService, testProvider: ITestProvider) {
	...
	store.add(vscode.commands.registerCommand('jiido.tests.fixTestFailure.fromInline', (item: vscode.TestItem) => {
	...
	store.add(vscode.commands.registerCommand('jiido.tests.fixTestFailure', openFixChat));
	...
	store.add(vscode.languages.registerCodeActionsProvider('*', {
		provideCodeActions(document, range, context, token) {
			...
			ca.command = {
				...
				command: 'jiido.tests.fixTestFailure',
				arguments: [{
